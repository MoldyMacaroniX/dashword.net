# TL;DR

You are allowed to use this codebase for your own project, but you have to remove the articles and any Dashword.net branding. You should also remove any Dashword.net Google Analytics / Adsense / other code specific to Dashword.net.

---

# Articles:

All articles are owned by their respective authors, they only give Dashword.net permission to publish them on our website and in this repository, as per our Author Terms: https://www.dashword.net/author-terms/

# Dashword.net branding, domain, and website:

The Dashword.net branding, domain, and website is owned by Moldy.

# Code:

The code is under the MIT license: https://opensource.org/license/mit
